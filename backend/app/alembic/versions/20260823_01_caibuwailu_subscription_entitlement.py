...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('active', 'revoked')", name="ck_cw_member_subscription_status"),
        sa.CheckConstraint("source IN ('manual', 'order', 'migration')", name="ck_cw_member_subscription_source"),
        sa.CheckConstraint("expires_at > starts_at", name="ck_cw_member_subscription_period"),
        sa.CheckConstraint("version_no >= 1", name="ck_cw_member_subscription_version"),
        sa.UniqueConstraint("uuid"),
        sa.UniqueConstraint("external_ref", name="uq_cw_member_subscription_external_ref"),
        comment="财不外露会员订阅",
    )
    op.create_index(
        "ix_cw_member_subscription_user_effective",
        "cw_member_subscription",
        ["user_id", "status", "starts_at", "expires_at", "plan_id"],
    )
    op.create_index(
        "ix_cw_member_subscription_plan_effective",
        "cw_member_subscription",
        ["plan_id", "status", "starts_at", "expires_at", "user_id"],
    )
