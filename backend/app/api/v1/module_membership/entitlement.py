...
EntitlementFailure = Literal["login_required", "membership_required", "plan_required"]
...
async def load_entitlement_context(
    ...
    *,
    ...
    if user_id is None:
        return EntitlementContext(user_id=None, active_plan_ids=frozenset(), subscriptions=())
...
def evaluate_content_access(
    *,
    ...
    if not context.is_authenticated:
        return EntitlementDecision(can_access=False, failure="login_required")
    if access_level == "login":
        return EntitlementDecision(can_access=True)
    if access_level == "member":
        if context.is_member:
            return EntitlementDecision(can_access=True)
        return EntitlementDecision(can_access=False, failure="membership_required")
...
async def count_active_members(db: AsyncSession, *, now: datetime | None = None) -> int:
    current = as_utc(now or utc_now())
