...
class MemberSubscriptionModel(ModelMixin, UserMixin):
    ...
    __table_args__ = (
        UniqueConstraint("source", "source_ref", name="uq_cw_member_subscription_source_ref"),
        CheckConstraint(
            "source IN ('manual', 'payment', 'migration', 'promotion')",
            name="ck_cw_member_subscription_source",
        ),
        CheckConstraint("status IN (0, 1)", name="ck_cw_member_subscription_status"),
        CheckConstraint("expires_at > starts_at", name="ck_cw_member_subscription_window"),
        ...
        Index("ix_cw_member_subscription_expiry", "status", "expires_at", "id"),
        {"comment": "财不外露用户会员订阅"},
    )
    ...
    source: Mapped[str] = mapped_column(String(32), nullable=False, comment="订阅来源")
    source_ref: Mapped[str] = mapped_column(String(128), nullable=False, comment="来源幂等键")
    status: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="状态(0生效 1撤销)")
    starts_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="生效时间")
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, comment="到期时间(排他)")
    revoked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, comment="撤销时间")
    grant_reason: Mapped[str] = mapped_column(String(500), nullable=False, comment="授权原因")
