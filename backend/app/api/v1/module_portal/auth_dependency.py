...
async def get_current_portal_user(
    ...
    if auth is None:
        raise CustomException(msg="请登录后继续", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
    return auth


async def _validate_portal_access_token(*, redis: Redis, token: str) -> dict[str, Any]:
