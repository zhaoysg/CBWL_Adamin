...
from .auth_schema import PortalAuthSessionResponse, PortalAuthUser, PortalCaptchaResponse, PortalLoginInput
...
class PortalAuthService:
    ...
    @classmethod
    async def issue_captcha(cls, request: Request, redis: Redis) -> PortalCaptchaResponse:
        ...
        if not settings.CAPTCHA_ENABLE:
            return PortalCaptchaResponse(enable=False, key="disabled", question=None)
        ...
        if not await RedisCURD(redis).set(
            ...
            raise CustomException(msg="安全验证服务暂不可用", status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
        return PortalCaptchaResponse(enable=True, key=key, question=question)
    ...
    async def login(
        ...
        *,
        ...
        await cls._consume_captcha(redis=redis, key=data.captcha_key, answer=data.captcha_answer)
    ...
    async def refresh(
        ...
        *,
        ...
        result = await LoginService.refresh_token(db=db, redis=redis, refresh_token=refresh_token)
        user = await cls._user_from_access_token(redis, result.access_token)
        return (
            PortalAuthSessionResponse(
                access_token=result.access_token,
                token_type=result.token_type,
    ...
    def validate_browser_origin(request: Request) -> None:
        ...
        if not raw_origin:
            raise CustomException(msg="缺少请求来源", code=RET.FORBIDDEN.code, status_code=status.HTTP_403_FORBIDDEN)
        try:
            origin = _canonical_origin(raw_origin)
        except (TypeError, ValueError) as exc:
            raise CustomException(msg="请求来源无效", code=RET.FORBIDDEN.code, status_code=status.HTTP_403_FORBIDDEN) from exc
        if origin not in portal_auth_settings.allowed_origins:
            raise CustomException(msg="请求来源未授权", code=RET.FORBIDDEN.code, status_code=status.HTTP_403_FORBIDDEN)

    @classmethod
    async def _consume_captcha(cls, *, redis: Redis, key: str | None, answer: str | None) -> None:
        if not settings.CAPTCHA_ENABLE:
            return
        if not key or not answer:
            raise CustomException(msg="请完成安全验证", status_code=status.HTTP_400_BAD_REQUEST)
        ...
        if not isinstance(expected, str) or not hmac.compare_digest(expected, actual):
            raise CustomException(msg="安全验证错误或已过期", status_code=status.HTTP_400_BAD_REQUEST)

        legacy_key = f"{RedisInitKeyConfig.CAPTCHA_CODES.key}:{key}"
        if not await redis_crud.set(legacy_key, "verified", expire=settings.CAPTCHA_EXPIRE_SECONDS):
            raise CustomException(msg="安全验证服务暂不可用", status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
    ...
    @classmethod
    async def _assert_current_refresh_token(cls, redis: Redis, token: str) -> dict[str, Any]:
        payload = decode_access_token(token=token, verify_exp=True)
        if not payload.is_refresh:
            raise CustomException(msg="非法刷新凭证", code=RET.INVALID_CREDENTIALS.code, status_code=status.HTTP_401_UNAUTHORIZED)
        redis_crud = RedisCURD(redis)
        ...
        if not isinstance(current, str) or not hmac.compare_digest(current, token):
            raise CustomException(msg="刷新凭证已失效", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
        session = await cls._session(redis_crud, payload.sub)
        cls._assert_portal_login_type(session)
        return session

    @classmethod
    async def _user_from_access_token(cls, redis: Redis, token: str) -> PortalAuthUser:
        payload = decode_access_token(token=token, verify_exp=True)
        if payload.is_refresh:
            raise CustomException(msg="非法访问凭证", code=RET.INVALID_CREDENTIALS.code, status_code=status.HTTP_401_UNAUTHORIZED)
        redis_crud = RedisCURD(redis)
        current = await redis_crud.get(f"{RedisInitKeyConfig.ACCESS_TOKEN.key}:{payload.sub}")
        if isinstance(current, bytes):
            current = current.decode("utf-8")
        if not isinstance(current, str) or not hmac.compare_digest(current, token):
            raise CustomException(msg="访问凭证已失效", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
        session = await cls._session(redis_crud, payload.sub)
        cls._assert_portal_login_type(session)
    ...
    @staticmethod
    async def _session(redis_crud: RedisCURD, session_id: str) -> dict[str, Any]:
        raw = await redis_crud.get(f"{RedisInitKeyConfig.USER_SESSION.key}:{session_id}")
        if isinstance(raw, bytes):
            raw = raw.decode("utf-8")
        if not isinstance(raw, str):
            raise CustomException(msg="会话已失效", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
        try:
            data = json.loads(raw)
        except json.JSONDecodeError as exc:
            raise CustomException(msg="会话已失效", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED) from exc
        if not isinstance(data, dict):
            raise CustomException(msg="会话已失效", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
        return data

    @staticmethod
    def _assert_portal_login_type(session: dict[str, Any]) -> None:
        if str(session.get("login_type") or "") not in portal_auth_settings.allowed_login_types:
            raise CustomException(msg="客户端会话类型不匹配", code=RET.UNAUTHORIZED.code, status_code=status.HTTP_401_UNAUTHORIZED)
    ...
    async def _enforce_rate_limit(
        *,
        ...
        try:
            ...
            logger.exception("Portal 登录限流服务异常: bucket={}", bucket)
            raise CustomException(msg="登录保护服务暂不可用", status_code=status.HTTP_503_SERVICE_UNAVAILABLE) from exc
        if current > limit:
            raise CustomException(
                msg="请求过于频繁，请稍后再试",
